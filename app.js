...
mongoose.connect('mongodb://localhost: 27017/saas-SUD-db',{ useNewUrlParser: true, useUnifiedTopology: true })
...
app.use(expressSession({
  secret:"hkdgkjdvhkjchfdmsdf,bvhsklmqdfkjdkslm<kfnsd,<;fn,"
}));
...
app.get('/', function(req, res, next){
  res.render('index', {title: "SaaS SUD"})
})
...
app.post('/signup', function(req, res, next){
  User.findOne({
    ...
    if (err) return next(err);
    if (user) return next({message: "oops, Sorry User exist already. Try Again "});
    let newUser = new User({
      email: req.body.email,
